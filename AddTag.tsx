...
const AddTag = ({
  ...
  action: (prevState: any, formData: any) => Promise<any>;
}) => {
  ...
  useEffect(() => {
    if (state?.success === true) {
      if (state?.data?.created_tags?.length > 0) {
        setCreatedTagsMsg(
          `Tags created successfully: ${state.data.created_tags.join(", ")}`,
        );
      }
    ...
    <div className="tab-content border border-solid border-[#ededed] rounded-2xl p-5.5">
      ...
      <div className="block">
        <div className="add-one-by-one_content">
          <h4 className="title h4 mb-5 text-[16px] font-semibold leading-5.5">
            Add Tags One-by-One
          </h4>
